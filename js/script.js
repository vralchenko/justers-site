document.addEventListener('DOMContentLoaded', () => {
    ...
    // -------------------------
    // Service Dropdown (Цивільні справи)
    // -------------------------
    ...
    const serviceDropdownInner = document.getElementById('serviceDropdownInner');
    const civilItem = document.querySelector('.service-item[data-service="civil"]');

    if (civilItem && serviceDropdown && serviceDropdownInner) {
        // Render dropdown content
        serviceDropdownInner.innerHTML = civilCategories.map(cat => `
            <div class="service-dropdown-card">
                <div class="service-dropdown-card-icon"><i class="${cat.icon}"></i></div>
                <h3>${cat.title}</h3>
                <ul>${cat.items.map(item => `<li>${item}</li>`).join('')}</ul>
            </div>
        `).join('');

        civilItem.addEventListener('click', () => {
